...
void testCreate() {
	...
	for (int i = -5; i < 5; i++) {
        cout << "Searching for " << i << " in empty bag." << endl;
...
void testAdd() {
    ...
    // Add initial values from 0 to 9
    cout << "Adding elements from 0 to 9" << endl;
    for (int i = 0; i < 10; i++) {
        cout << "Adding " << i << endl;
        b.add(i);
    }
    cout << "Bag state after adding 0 to 9: size = " << b.size() << ", isEmpty = " << b.isEmpty() << endl;
    assert(b.isEmpty() == false);
    assert(b.size() == 10);

    // Add elements from -10 to 19
    cout << "Adding elements from -10 to 19" << endl;
    for (int i = -10; i < 20; i++) {
        cout << "Adding " << i << endl;
        b.add(i);
    }
    cout << "Bag state after adding -10 to 19: size = " << b.size() << ", isEmpty = " << b.isEmpty() << endl;
    assert(b.isEmpty() == false);
    assert(b.size() == 40);

    // Add elements from -100 to 99
    cout << "Adding elements from -100 to 99" << endl;
    for (int i = -100; i < 100; i++) {
        cout << "Adding " << i << endl;
        b.add(i);
    }
    cout << "Bag state after adding -100 to 99: size = " << b.size() << ", isEmpty = " << b.isEmpty() << endl;
    assert(b.isEmpty() == false);
    assert(b.size() == 240);

    // Test iterator steps
    cout << "Testing iterator steps after adding all elements" << endl;
    testIteratorSteps(b);

    // Verify occurrences and search results
    cout << "Verifying occurrences and search results for elements from -200 to 199" << endl;
    for (int i = -200; i < 200; i++) {
        int count = b.nrOccurrences(i);
        cout << "Element: " << i << ", Occurrences: " << count << endl;
        if (i < -100) {
            assert(count == 0);
            assert(b.search(i) == false);
        }
    ...
    // Add large range of elements from 10000 down to -9999
    cout << "Adding elements from 10000 down to -9999" << endl;
    for (int i = 10000; i > -10000; i--) {
        b.add(i);
    }
    cout << "Bag state after adding 10000 to -9999: size = " << b.size() << endl;
    assert(b.size() == 20240);

    // Test iterator steps again after large addition
    cout << "Testing iterator steps after adding range 10000 to -9999" << endl;
    testIteratorSteps(b);
}
void testRemove() {
